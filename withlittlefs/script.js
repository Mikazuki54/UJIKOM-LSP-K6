...
// Fungsi Menarik Data JSON secara relatif
function fetchSensorData() {
    fetch('/data') // Cukup panggil /data, browser sudah tau IP-nya
        ...
        .catch(error => console.log("Menunggu koneksi data dari ESP32..."));
}

setInterval(fetchSensorData, 2000);
fetchSensorData();

// Fungsi Kontrol Aktuator secara relatif
let fanState = false;
let pumpState = false;
...
function toggleFan() {
    ...
    fetch(`/fan?state=${stateParam}`) // Memanggil ke IP ESP32 secara otomatis
        .then(response => {
            if (response.ok) {
...
function togglePump() {
    ...
    fetch(`/pump?state=${stateParam}`) // Memanggil ke IP ESP32 secara otomatis
        .then(response => {
            if (response.ok) {
